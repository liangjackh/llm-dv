...
class riscv_illegal_instr extends uvm_object;
  ...
  // Avoid generating illegal func3/func7 for opcodes used by the P-extension.
  // OP-32 (0111011) and OP-IMM-32 (0011011) carry the packed-SIMD w/f encoding
  // scheme, and OP-IMM (0010011) is shared with ABS/CLS's Zbb-style func7+func5
  // constant encoding (0010011 is also excluded by zbb_extension_c when Zbb is
  // enabled, but P can be enabled without Zbb, so it needs its own exclusion).
  constraint p_extension_c {
    if (RV32P inside {supported_isa} || RV64P inside {supported_isa}) {
      if (exception inside {kIllegalFunc3, kIllegalFunc7}) {
        !(opcode inside {7'b0111011, 7'b0011011, 7'b0010011});
      }
    }
  }
