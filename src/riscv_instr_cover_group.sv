...
`define SAMPLE_P(cg, val) `SAMPLE_W_TYPE(cg, val, riscv_p_instr)
...
`define P_R_INSTR_CG_BEGIN(INSTR_NAME) \
  `INSTR_CG_BEGIN(INSTR_NAME, riscv_p_instr) \
    cp_rs1         : coverpoint instr.rs1; \
    cp_rs2         : coverpoint instr.rs2; \
    cp_rd          : coverpoint instr.rd;  \
    `DV(cp_gpr_hazard : coverpoint instr.gpr_hazard;) \

`define P_I_INSTR_CG_BEGIN(INSTR_NAME) \
  `INSTR_CG_BEGIN(INSTR_NAME, riscv_p_instr) \
    cp_rs1         : coverpoint instr.rs1; \
    cp_rd          : coverpoint instr.rd; \
    `DV(cp_gpr_hazard : coverpoint instr.gpr_hazard;) \
...
class riscv_instr_cover_group;
  ...
  // RV32P/RV64P instructions (Packed-SIMD / DSP extension, core subset)
  `P_R_INSTR_CG_BEGIN(padd_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(padd_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psadd_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psadd_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(sadd)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psaddu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psaddu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(saddu)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psub_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psub_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssub_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssub_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(ssub)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssubu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssubu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(ssubu)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paadd_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paadd_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(aadd)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paaddu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paaddu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(aaddu)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasub_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasub_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(asub)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasubu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasubu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(asubu)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pabd_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pabd_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pabdu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pabdu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmin_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmin_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pminu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pminu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmax_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmax_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmaxu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmaxu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmseq_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmseq_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(mseq)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmslt_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmslt_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(mslt)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmsltu_b)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmsltu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(msltu)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psll_bs)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psll_hs)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psrl_bs)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psrl_hs)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psra_bs)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psra_hs)
  `CG_END

  `P_I_INSTR_CG_BEGIN(pslli_b)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 7)
  `CG_END

  `P_I_INSTR_CG_BEGIN(pslli_h)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 15)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrli_b)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 7)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrli_h)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 15)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrai_b)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 7)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrai_h)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 15)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulh_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulhu_h)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulhsu_h)
  `CG_END

  `P_I_INSTR_CG_BEGIN(abs)
  `CG_END

  `P_I_INSTR_CG_BEGIN(cls)
  `CG_END

  `P_R_INSTR_CG_BEGIN(padd_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psadd_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psaddu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psub_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssub_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pssubu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paadd_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(paaddu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasub_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pasubu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmin_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pminu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmax_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmaxu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmseq_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmslt_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmsltu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psll_ws)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psrl_ws)
  `CG_END

  `P_R_INSTR_CG_BEGIN(psra_ws)
  `CG_END

  `P_I_INSTR_CG_BEGIN(pslli_w)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 31)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrli_w)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 31)
  `CG_END

  `P_I_INSTR_CG_BEGIN(psrai_w)
    `CP_VALUE_RANGE(shift_amt, instr.imm, 0, 31)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulh_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulhu_w)
  `CG_END

  `P_R_INSTR_CG_BEGIN(pmulhsu_w)
  `CG_END

  `P_I_INSTR_CG_BEGIN(absw)
  `CG_END

  `P_I_INSTR_CG_BEGIN(clsw)
  `CG_END
  ...
  */
  ...
  function void sample(riscv_instr instr);
    ...
    case (instr.instr_name)
      ...
      // RV32P/RV64P instructions (Packed-SIMD / DSP extension, core subset)
      PADD_B       : `SAMPLE_P(padd_b_cg, instr)
      PADD_H       : `SAMPLE_P(padd_h_cg, instr)
      PSADD_B      : `SAMPLE_P(psadd_b_cg, instr)
      PSADD_H      : `SAMPLE_P(psadd_h_cg, instr)
      SADD         : `SAMPLE_P(sadd_cg, instr)
      PSADDU_B     : `SAMPLE_P(psaddu_b_cg, instr)
      PSADDU_H     : `SAMPLE_P(psaddu_h_cg, instr)
      SADDU        : `SAMPLE_P(saddu_cg, instr)
      PSUB_B       : `SAMPLE_P(psub_b_cg, instr)
      PSUB_H       : `SAMPLE_P(psub_h_cg, instr)
      PSSUB_B      : `SAMPLE_P(pssub_b_cg, instr)
      PSSUB_H      : `SAMPLE_P(pssub_h_cg, instr)
      SSUB         : `SAMPLE_P(ssub_cg, instr)
      PSSUBU_B     : `SAMPLE_P(pssubu_b_cg, instr)
      PSSUBU_H     : `SAMPLE_P(pssubu_h_cg, instr)
      SSUBU        : `SAMPLE_P(ssubu_cg, instr)
      PAADD_B      : `SAMPLE_P(paadd_b_cg, instr)
      PAADD_H      : `SAMPLE_P(paadd_h_cg, instr)
      AADD         : `SAMPLE_P(aadd_cg, instr)
      PAADDU_B     : `SAMPLE_P(paaddu_b_cg, instr)
      PAADDU_H     : `SAMPLE_P(paaddu_h_cg, instr)
      AADDU        : `SAMPLE_P(aaddu_cg, instr)
      PASUB_B      : `SAMPLE_P(pasub_b_cg, instr)
      PASUB_H      : `SAMPLE_P(pasub_h_cg, instr)
      ASUB         : `SAMPLE_P(asub_cg, instr)
      PASUBU_B     : `SAMPLE_P(pasubu_b_cg, instr)
      PASUBU_H     : `SAMPLE_P(pasubu_h_cg, instr)
      ASUBU        : `SAMPLE_P(asubu_cg, instr)
      PABD_B       : `SAMPLE_P(pabd_b_cg, instr)
      PABD_H       : `SAMPLE_P(pabd_h_cg, instr)
      PABDU_B      : `SAMPLE_P(pabdu_b_cg, instr)
      PABDU_H      : `SAMPLE_P(pabdu_h_cg, instr)
      PMIN_B       : `SAMPLE_P(pmin_b_cg, instr)
      PMIN_H       : `SAMPLE_P(pmin_h_cg, instr)
      PMINU_B      : `SAMPLE_P(pminu_b_cg, instr)
      PMINU_H      : `SAMPLE_P(pminu_h_cg, instr)
      PMAX_B       : `SAMPLE_P(pmax_b_cg, instr)
      PMAX_H       : `SAMPLE_P(pmax_h_cg, instr)
      PMAXU_B      : `SAMPLE_P(pmaxu_b_cg, instr)
      PMAXU_H      : `SAMPLE_P(pmaxu_h_cg, instr)
      PMSEQ_B      : `SAMPLE_P(pmseq_b_cg, instr)
      PMSEQ_H      : `SAMPLE_P(pmseq_h_cg, instr)
      MSEQ         : `SAMPLE_P(mseq_cg, instr)
      PMSLT_B      : `SAMPLE_P(pmslt_b_cg, instr)
      PMSLT_H      : `SAMPLE_P(pmslt_h_cg, instr)
      MSLT         : `SAMPLE_P(mslt_cg, instr)
      PMSLTU_B     : `SAMPLE_P(pmsltu_b_cg, instr)
      PMSLTU_H     : `SAMPLE_P(pmsltu_h_cg, instr)
      MSLTU        : `SAMPLE_P(msltu_cg, instr)
      PSLL_BS      : `SAMPLE_P(psll_bs_cg, instr)
      PSLL_HS      : `SAMPLE_P(psll_hs_cg, instr)
      PSRL_BS      : `SAMPLE_P(psrl_bs_cg, instr)
      PSRL_HS      : `SAMPLE_P(psrl_hs_cg, instr)
      PSRA_BS      : `SAMPLE_P(psra_bs_cg, instr)
      PSRA_HS      : `SAMPLE_P(psra_hs_cg, instr)
      PSLLI_B      : `SAMPLE_P(pslli_b_cg, instr)
      PSLLI_H      : `SAMPLE_P(pslli_h_cg, instr)
      PSRLI_B      : `SAMPLE_P(psrli_b_cg, instr)
      PSRLI_H      : `SAMPLE_P(psrli_h_cg, instr)
      PSRAI_B      : `SAMPLE_P(psrai_b_cg, instr)
      PSRAI_H      : `SAMPLE_P(psrai_h_cg, instr)
      PMULH_H      : `SAMPLE_P(pmulh_h_cg, instr)
      PMULHU_H     : `SAMPLE_P(pmulhu_h_cg, instr)
      PMULHSU_H    : `SAMPLE_P(pmulhsu_h_cg, instr)
      ABS          : `SAMPLE_P(abs_cg, instr)
      CLS          : `SAMPLE_P(cls_cg, instr)
      PADD_W       : `SAMPLE_P(padd_w_cg, instr)
      PSADD_W      : `SAMPLE_P(psadd_w_cg, instr)
      PSADDU_W     : `SAMPLE_P(psaddu_w_cg, instr)
      PSUB_W       : `SAMPLE_P(psub_w_cg, instr)
      PSSUB_W      : `SAMPLE_P(pssub_w_cg, instr)
      PSSUBU_W     : `SAMPLE_P(pssubu_w_cg, instr)
      PAADD_W      : `SAMPLE_P(paadd_w_cg, instr)
      PAADDU_W     : `SAMPLE_P(paaddu_w_cg, instr)
      PASUB_W      : `SAMPLE_P(pasub_w_cg, instr)
      PASUBU_W     : `SAMPLE_P(pasubu_w_cg, instr)
      PMIN_W       : `SAMPLE_P(pmin_w_cg, instr)
      PMINU_W      : `SAMPLE_P(pminu_w_cg, instr)
      PMAX_W       : `SAMPLE_P(pmax_w_cg, instr)
      PMAXU_W      : `SAMPLE_P(pmaxu_w_cg, instr)
      PMSEQ_W      : `SAMPLE_P(pmseq_w_cg, instr)
      PMSLT_W      : `SAMPLE_P(pmslt_w_cg, instr)
      PMSLTU_W     : `SAMPLE_P(pmsltu_w_cg, instr)
      PSLL_WS      : `SAMPLE_P(psll_ws_cg, instr)
      PSRL_WS      : `SAMPLE_P(psrl_ws_cg, instr)
      PSRA_WS      : `SAMPLE_P(psra_ws_cg, instr)
      PSLLI_W      : `SAMPLE_P(pslli_w_cg, instr)
      PSRLI_W      : `SAMPLE_P(psrli_w_cg, instr)
      PSRAI_W      : `SAMPLE_P(psrai_w_cg, instr)
      PMULH_W      : `SAMPLE_P(pmulh_w_cg, instr)
      PMULHU_W     : `SAMPLE_P(pmulhu_w_cg, instr)
      PMULHSU_W    : `SAMPLE_P(pmulhsu_w_cg, instr)
      ABSW         : `SAMPLE_P(absw_cg, instr)
      CLSW         : `SAMPLE_P(clsw_cg, instr)
